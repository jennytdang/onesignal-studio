...
export default function CanvasPreview({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const scaleX = (width - 64) / cw
      const scaleY = (height - 64) / ch
    ...
    >
      ...
      {template === 'newhire' && totalSlides > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          >← Prev</button>
          <span style={{ color: '#59626B', fontSize: 13, fontFamily: "'Nunito Sans', sans-serif" }}>
            Slide {slideIndex + 1} of {totalSlides}
          </span>
          <button
            onClick={() => setSlideIndex(Math.min(totalSlides - 1, slideIndex + 1))}
            disabled={slideIndex === totalSlides - 1}
            style={navBtn(slideIndex === totalSlides - 1)}
          >Next →</button>
        </div>
      )}

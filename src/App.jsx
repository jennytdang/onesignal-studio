...
const TEMPLATE_DEFAULTS = {
  ...
  newhire:  { pill:'', headline:'Meet our newest members!', subheadline:'Welcome to the team! 🎉', showHiring:false, cta:'', stat:'', statLabel:'', authorName:'', authorTitle:'', authorCompany:'', showHeadshot:false, headshotUrl:'', eventDate:'', eventLocation:'', speakers:[{name:'',title:'',company:'',photo:''}], emoji:'🎉', quoteBlock:false },
}
...
function NewHireFields({ fields, update, newHireSlides, setNewHireSlides, dimension }) {
  ...
  return (<><SectionLabel>Cover Slide</SectionLabel><Input label="Headline" value={fields.headline} onChange={v=>update('headline',v)} placeholder="Meet our newest members!"/><Input label="Subheadline" value={fields.subheadline} onChange={v=>update('subheadline',v)} placeholder="Welcome to the team!"/>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:10,marginBottom:2}}>
          <span style={{fontSize:13,color:T.textSub,fontFamily:"'Nunito Sans', sans-serif"}}>Show "(We're hiring)"</span>
          <button onClick={()=>update('showHiring',!fields.showHiring)} style={{width:36,height:20,borderRadius:10,border:'none',cursor:'pointer',background:fields.showHiring?T.purple:'#CBD0D5',position:'relative',transition:'background 0.2s',flexShrink:0}}>
            <span style={{position:'absolute',top:2,left:fields.showHiring?18:2,width:16,height:16,borderRadius:'50%',background:'#fff',transition:'left 0.2s',display:'block'}}/>
          </button>
        </div>

...
function Input({ label, value, onChange, placeholder, multiline, rows=3, limit, disabled }) {
  ...
  const style = { width:'100%', background:disabled?T.bgHover:T.bgInput, color:disabled?'#98A1A9':T.text, WebkitTextFillColor:disabled?'#98A1A9':T.text, opacity:1, cursor:disabled?'not-allowed':'text', border:`1px solid ${limit&&(value||'').length>limit?'#E24B4A':focused?T.borderFocus:T.border}`, borderRadius:2, padding:'9px 12px', color:T.text, fontSize:13, fontFamily:"'Nunito Sans', sans-serif", outline:'none', resize:multiline?'vertical':'none', lineHeight:1.5, boxSizing:'border-box', transition:'border-color 0.15s' }
  return (
        ...
        <div className="sidebar-scroll" style={{flex:1,overflowY:'auto',padding:'14px 14px 24px'}}>
          ...
          {template !== 'quote' && template !== 'event' && template !== 'newhire' && (<>
            ...
            <div style={{display:'flex',gap:6,marginBottom:14}}>
              {['left','center'].map(a=><button key={a} onClick={()=>setLogoAlign(a)} style={{flex:1,padding:'7px 0',fontSize:12,fontFamily:"'Epilogue',sans-serif",background:logoAlign===a?T.purple50:'transparent',color:logoAlign===a?T.purple:T.textMuted,fontWeight:logoAlign===a?500:400,border:`1px solid ${logoAlign===a?T.purple:T.border}`,borderRadius:2,cursor:'pointer',textTransform:'capitalize',fontWeight:logoAlign===a?500:400}}>{a}</button>)}
            </div>
          </>)}
          <Divider/>

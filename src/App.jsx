...
function NewHireFields({ fields, update, newHireSlides, setNewHireSlides, dimension }) {
  ...
  const remP=(si,pi)=>{const sl=[...newHireSlides];sl[si]=sl[si].filter((_,j)=>j!==pi);const cleaned=sl.filter(s=>s.length>0);setNewHireSlides(cleaned.length>0?cleaned:[[emptyPerson()]]);},cleaned.length>0?cleaned:[[emptyPerson()]]);},addP=()=>{const sl=newHireSlides.length>0?[...newHireSlides]:[[{...emptyPerson()}]];const last=[...(sl[sl.length-1]||[])];if(last.length<9){last.push(emptyPerson());sl[sl.length-1]=last}else{sl.push([emptyPerson()])};setNewHireSlides(sl)}

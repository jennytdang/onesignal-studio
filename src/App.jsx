  ...
  return (
    <div style={{marginBottom:12}}>
      ...
      <div style={{position:'relative'}}>
        <input type="text" value={fields.pill} onChange={e=>update('pill',e.target.value)} placeholder="e.g. Webinar, New Featureâ¦" onFocus={()=>setPillOpen(true)} onBlur={()=>setTimeout(()=>setPillOpen(false),150)} style={{width:'100%',background:T.bgInput,border:`1px solid ${T.border}`,borderRadius:2,padding:'9px 12px',color:T.text,fontSize:13,fontFamily:"'Nunito Sans', sans-serif",outline:'none',boxSizing:'border-box'}}/>
...
function StatFields({ fields, update }) { return (<><CommonFields fields={fields} update={update}/><Input label="Stat / Number *" value={fields.stat} onChange={v=>update('stat',v)} placeholder="e.g. 2Ã, 98%, 10M"/><Input label="Stat Label" value={fields.statLabel} onChange={v=>update('statLabel',v)} placeholder="e.g. faster delivery"/><Input label="Supporting Copy" value={fields.subheadline} onChange={v=>update('subheadline',v)} placeholder="Context for the statâ¦" multiline rows={3}/><Input label="CTA Button" value={fields.cta} onChange={v=>update('cta',v)} placeholder="e.g. Read the report" limit={CTA_LIMIT}/></>) }

function QuoteFields({ fields, update }) {
  return (<>
    <CommonFields fields={fields} update={update}/>
    <Input label="Quote *" value={fields.headline} onChange={v=>update('headline',v)} placeholder="The quote text goes hereâ¦" multiline rows={4}/>
    <Divider/>
      ...
      <div style={{width:252,flexShrink:0,display:'flex',flexDirection:'column',background:T.bgSurface,borderRight:`1px solid ${T.border}`,height:'100vh',overflow:'hidden'}}>
        ...
        <div style={{padding:14,borderTop:`1px solid ${T.border}`}}>
          ...
          >
            ...
            <span style={{position:'relative',zIndex:2}}>{exporting?'Exportingâ¦':template==='newhire'?'â Export PDF Carousel':'â Export JPG'}</span>
          </button>
          <div style={{textAlign:'center',marginTop:6,fontSize:11,color:T.textMuted,fontFamily:"'Nunito Sans', sans-serif"}}>{template==='newhire'?`${totalSlides} slides Â· LinkedIn carousel`:`${dimension.width} Ã ${dimension.height}px Â· JPG`}</div>
        </div>
      </div>
      <div style={{flex:1,display:'flex',flexDirection:'column',background:T.bgPage,overflow:'hidden'}}>
        <div style={{height:44,borderBottom:`1px solid ${T.border}`,background:T.bgSurface,display:'flex',alignItems:'center',padding:'0 20px',flexShrink:0}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            ...
            <span style={{fontSize:11,fontWeight:500,color:'#4E50D1',background:'#F3F3FC',borderRadius:2,padding:'3px 8px',fontFamily:"'SF Mono','Fira Code','Cascadia Code',monospace",letterSpacing:'-0.02em'}}>{dimension.width} Ã {dimension.height}</span>
            <span style={{fontSize:12,color:T.textSub,fontFamily:"'Nunito Sans', sans-serif"}}>
              {TEMPLATES.find(t=>t.id===template)?.label}
              <span style={{margin:'0 4px',color:T.border}}>Â·</span>
              {background.label}
            </span>
          </div>

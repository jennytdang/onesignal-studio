...
const TEMPLATE_DEFAULTS = {
  headline: { pill:'New Product Feature', headline:'Introducing OneSignal Events', subheadline:'Act on every moment that matters', cta:'Learn more about Events', stat:'200k+', statLabel:'', authorName:'', authorTitle:'', authorCompany:'', showHeadshot:false, headshotUrl:'', eventDate:'', eventLocation:'', speakers:[{name:'',title:'',company:'',photo:''}], emoji:'🎉', quoteBlock:false },
  stat:     { pill:'', headline:'', subheadline:'OneSignal now empowers over a million businesses to send 12 billion messages daily.', cta:'Get started now', stat:'200k+', statLabel:'Apps powered by OneSignal', authorName:'', authorTitle:'', authorCompany:'', showHeadshot:false, headshotUrl:'', eventDate:'', eventLocation:'', speakers:[{name:'',title:'',company:'',photo:''}], emoji:'🎉', quoteBlock:false },
  quote:    { pill:'', headline:'OneSignal aims to democratize customer engagement by empowering businesses with omnichannel experiences, saving time and money through a comprehensive platform.', subheadline:'', cta:'', stat:'', statLabel:'', authorName:'George Deglin', authorTitle:'Co-Founder & CEO', authorCompany:'OneSignal', showHeadshot:false, headshotUrl:'', eventDate:'', eventLocation:'', speakers:[{name:'',title:'',company:'',photo:''}], emoji:'🎉', quoteBlock:false },
  ...
  newhire:  { pill:'', headline:'Meet our newest members!', subheadline:'(p.s. we\'re hiring)', showIntroSlide:true, cta:'', stat:'', statLabel:'', authorName:'', authorTitle:'', authorCompany:'', showHeadshot:false, headshotUrl:'', eventDate:'', eventLocation:'', speakers:[{name:'',title:'',company:'',photo:''}], emoji:'🎉', quoteBlock:false },
}
...
function EventFields({ fields, update }) {
  ...
  const add=()=>{const s=[...(fields.speakers||[])];s.push({name:'',title:'',company:'',photo:''});update('speakers',s)},
  rem=(i)=>{const s=(fields.speakers||[]).filter((_,j)=>j!==i);update('speakers',s.length>0?s:[{name:'',title:'',company:'',photo:''}])}
  const spk=fields.speakers||[]
  return (<><Input label="Title *" tooltip="Use sentence case." value={fields.headline} onChange={v=>update('headline',v)} placeholder="Event title"/><Divider/><SectionLabel>Event Details</SectionLabel><Input label="Date & Time" value={fields.eventDate} onChange={v=>update('eventDate',v)} placeholder="March 26 @ 9:00AM PDT"/><Input label="Location" value={fields.eventLocation} onChange={v=>update('eventLocation',v)} placeholder="Virtual / San Francisco, CA"/><Input label="CTA Button" tooltip="Use sentence case and an action verb." value={fields.cta} onChange={v=>update('cta',v)} placeholder="Save your spot now" limit={CTA_LIMIT}/><Divider/><SectionLabel>Speakers</SectionLabel>{spk.map((s,i)=><div key={i} style={{background:T.bgPage,border:`1px solid ${T.border}`,borderRadius:2,padding:12,marginBottom:10}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}><span style={{fontSize:12,color:'#74808B',fontFamily:"'Nunito Sans', sans-serif"}}>Speaker {i+1}</span><button onClick={()=>rem(i)} style={{background:'none',border:'none',cursor:'pointer',padding:0,display:'flex',alignItems:'center'}}><img src="/trash-icon.svg" style={{width:16,height:16,filter:'brightness(0) saturate(100%) invert(68%) sepia(8%) saturate(394%) hue-rotate(167deg) brightness(96%) contrast(88%)'}} /></button></div><PhotoUpload value={s.photo} onChange={v=>upd(i,'photo',v)}/><Input value={s.name} onChange={v=>upd(i,'name',v)} placeholder="Name"/><Input value={s.title} onChange={v=>upd(i,'title',v)} placeholder="Title"/><Input value={s.company} onChange={v=>upd(i,'company',v)} placeholder="Company"/></div>)}{spk.length<4&&<button onClick={add} style={{width:'100%',background:'none',border:`1px dashed ${T.border}`,borderRadius:2,padding:'9px',color:T.textMuted,cursor:'pointer',fontSize:13,fontFamily:"'Nunito Sans', sans-serif"}}>+ Add speaker</button>}</>)
}

function NewHireFields({ fields, update, newHireSlides, setNewHireSlides, dimension }) {
  ...
  return (<><SectionLabel>Cover Slide</SectionLabel><Input label="Title" tooltip="Use sentence case." value={fields.headline} onChange={v=>update('headline',v)} placeholder="Meet our newest members!" disabled={fields.showIntroSlide===false}/><Input label="Description" value={fields.subheadline} onChange={v=>update('subheadline',v)} placeholder="Welcome to the team!" disabled={fields.showIntroSlide===false}/>
        ...
        <Divider/><SectionLabel>New Hires ({total})</SectionLabel>{newHireSlides.map((slide,si)=><div key={si}>{newHireSlides.length>1&&<div style={{fontSize:11,color:T.textMuted,marginBottom:8,textTransform:'uppercase',letterSpacing:'0.08em'}}>Grid Slide {si+1}</div>}{slide.map((p,pi)=><div key={pi} style={{background:T.bgPage,border:`1px solid ${T.border}`,borderRadius:2,padding:12,marginBottom:8}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}><span style={{fontSize:12,color:'#74808B',fontFamily:"'Nunito Sans', sans-serif"}}>Person {pi+1}</span><button onClick={()=>remP(si,pi)} style={{background:'none',border:'none',cursor:'pointer',padding:0,display:'flex',alignItems:'center'}}><img src="/trash-icon.svg" style={{width:16,height:16,filter:'brightness(0) saturate(100%) invert(68%) sepia(8%) saturate(394%) hue-rotate(167deg) brightness(96%) contrast(88%)'}} /></button></div><PhotoUpload value={p.photo} onChange={v=>updP(si,pi,'photo',v)}/><Input value={p.name} onChange={v=>updP(si,pi,'name',v)} placeholder="Full name"/><Input value={p.title} onChange={v=>updP(si,pi,'title',v)} placeholder="Job title"/></div>)}</div>)}{newHireSlides.flat().filter(p=>p.name).length<(dimension?.id==='landscape'?8:6)&&<button onClick={addP} style={{width:'100%',background:'none',border:`1px dashed ${T.border}`,borderRadius:2,padding:'9px',color:T.textMuted,cursor:'pointer',fontSize:13,fontFamily:"'Nunito Sans', sans-serif",marginBottom:8}}>+ Add person</button>}{newHireSlides.flat().filter(p=>p.name).length>=(dimension?.id==='landscape'?8:6)&&<div style={{fontSize:11,color:T.textMuted,textAlign:'center',padding:'6px 0',fontFamily:"'Nunito Sans', sans-serif"}}>{dimension?.id==='landscape'?'Max 8 people':'Max 6 people'}</div>}</>)
}
...
function FieldsPanel({ template, fields, update, newHireSlides, setNewHireSlides, dimension }) {
  ...
  if (template === 'newhire')  return <NewHireFields fields={fields} update={update} newHireSlides={newHireSlides} setNewHireSlides={setNewHireSlides} dimension={dimension}/>
  return null
}
...
export default function App() {
  ...
  const showIntroSlide = fields.showIntroSlide !== false
        ...
        <div className="sidebar-scroll" style={{flex:1,overflowY:'auto',padding:'14px 14px 24px'}}>
          ...
          {template !== 'quote' && template !== 'event' && template !== 'newhire' && (<>
            ...
            <div style={{display:'flex',gap:6,marginBottom:14}}>
              {['left','center'].map(a=><button key={a} onClick={()=>setLogoAlign(a)} style={{flex:1,padding:'7px 0',fontSize:12,fontFamily:"'Epilogue',sans-serif",background:logoAlign===a?T.purple50:'transparent',color:logoAlign===a?T.purple:T.textMuted,fontWeight:logoAlign===a?500:400,border:`1px solid ${logoAlign===a?T.purple:T.border}`,borderRadius:2,cursor:'pointer',textTransform:'capitalize'}}>{a}</button>)}
            </div>
          </>)}
          <Divider/>

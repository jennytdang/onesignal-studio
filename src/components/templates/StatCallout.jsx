...
export default function StatCallout({ fields, dimension, isDark, logoAlign = 'left', backgroundId }) {
  ...
  const fgSub       = ['grad-1','grad-4','white'].includes(backgroundId) ? '#24272B' : '#ffffff'
  ...
  const statFontSize = id === 'square' ? 220 : id === 'landscape' ? 238 : Math.round(S.statSize(width, height))
...
{id === 'landscape' ? (
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 48, justifyContent: logoAlign==='center'?'center':'flex-start', width: '100%' }}>
              ...
              {statLabel && <div style={{ fontSize: id==='story'?56:Math.round(height*0.05), fontWeight: 600, color: accentColor, letterSpacing: '-0.01em', wordBreak: 'break-word', overflowWrap: 'break-word', lineHeight: id==='landscape'?1.25:1.1, flexShrink: 1 }}>{statLabel}</div>}
            </div>
          ) : (
            <>
              ...
              {statLabel && (
                <div style={{ fontSize: id==='square'?42:id==='portrait'?50:id==='story'?56:Math.round(height*0.0537), fontWeight: 600, color: accentColor, marginTop: Math.round(height * 0.01), letterSpacing: '-0.01em', textAlign: logoAlign==='center'?'center':'left', maxWidth: '100%', wordBreak: 'break-word', overflowWrap: 'break-word' }}>{statLabel}</div>
              )}
            </>
          )}
        </div>

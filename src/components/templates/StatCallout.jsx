...
export default function StatCallout({ fields, dimension, isDark }) {
  ...
  const statFontSize = isStory ? Math.round(width * 0.26) : isLandscape ? Math.round(height * 0.28) : Math.round(width * 0.26)
  ...
  return (
    <div style={{ width, height, position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: pad, fontFamily: "'Epilogue', sans-serif" }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: lineBreakStat ? 'row' : 'column', alignItems: lineBreakStat ? 'center' : 'flex-start', gap: lineBreakStat ? Math.round(width * 0.06) : Math.round(height * 0.03), justifyContent: 'center' }}>
        <div>

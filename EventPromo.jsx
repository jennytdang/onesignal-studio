  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: Math.round(size * 0.018) }}>
      <div style={{
        ...
        border: `2px solid ${isDark ? 'rgba(255,255,255,0.15)' : COLORS.gray100}`,
      }}>
        {speaker.photo
          ? <img src={speaker.photo} alt={speaker.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          : <div style={{
              width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: Math.round(avatarSize * 0.38), color: COLORS.purple600,
              fontFamily: "'Epilogue', sans-serif", fontWeight: 700,
            }}>{speaker.name ? speaker.name.charAt(0) : '?'}</div>
        }
      </div>
      <div>
        <div style={{ fontSize: Math.round(size * (compact ? 0.022 : 0.026)), fontWeight: 700, color: fg, fontFamily: "'Epilogue', sans-serif", lineHeight: 1.2 }}>
          {speaker.name}
        </div>
        <div style={{ fontSize: Math.round(size * 0.019), color: fgSub, fontFamily: "'Nunito Sans', sans-serif", fontWeight: 400, lineHeight: 1.3 }}>
...
export default function EventPromo({ fields, dimension, isDark }) {
  ...
  const headlineSize = isStory ? Math.round(width * 0.082)
    : isLandscape ? Math.round(height * 0.088)
    : Math.round(width * 0.082)

  const validSpeakers = speakers.filter(s => s.name)
  const hasSpeakers = validSpeakers.length > 0

  if (isLandscape && hasSpeakers) {
    return (
      <div style={{ width, height, display: 'flex', fontFamily: "'Epilogue', sans-serif" }}>
        <div style={{ flex: '0 0 58%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: pad }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: Math.round(height * 0.028) }}>
            {pill && (
              <div style={{ display: 'inline-flex', alignSelf: 'flex-start', border: `1.5px solid ${pillBorder}`, borderRadius: 999, padding: `${Math.round(height * 0.012)}px ${Math.round(width * 0.022)}px`, color: pillText, fontSize: Math.round(width * 0.018), fontFamily: "'Nunito Sans', sans-serif", fontWeight: 600 }}>{pill}</div>
            )}
  ...
  const speakerCols = !isStory && validSpeakers.length > 2 ? 2 : 1
  ...
  return (
    <div style={{ width, height, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: pad, fontFamily: "'Epilogue', sans-serif" }}>
      ...
      {hasSpeakers && (
        <div>
          <div style={{ height: 1, backgroundColor: dividerColor, marginBottom: Math.round(height * 0.03) }} />
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${speakerCols}, 1fr)`, gap: Math.round(height * 0.028) }}>
            {validSpeakers.map((s, i) => <SpeakerCard key={i} speaker={s} size={width} isDark={isDark} />)}
          </div>
        </div>
      )}
    </div>
